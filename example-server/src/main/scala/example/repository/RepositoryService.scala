...
case class AlreadyDeleted(id: Long)
...
trait RepositoryService {
  ...
  def deleteOrder(id: Long): IO[AlreadyDeleted | UnknownOrder, Unit]
}
...
case class InMemoryRepositoryService(inventory: Ref[Map[String, Int]], orders: Ref[Map[Long, Order]]) extends RepositoryService {
  ...
  override def placeOrder(id: Long, petId: Long, quantity: Int): IO[PlaceOrderError, Order] =
    orders.modify { all =>
